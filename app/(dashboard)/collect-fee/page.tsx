...
export default async function CollectFeePage() {
  ...
  const [{ data: enrollmentsRaw }, { data: feeStructure }] = await Promise.all([
    supabase
      ...
      .select(`
        ...
        payments ( id, fee_head, amount, mode, payment_date, reference, receipt_no, remarks, created_at )
      `)
  ...
  const students: CollectFeeStudent[] = enrollments
    ...
    .map(e => {
      ...
      const rawPayments = (e.payments as unknown as {
        id: string; fee_head: string; amount: number; mode: string
        payment_date: string; reference: string | null; receipt_no: string
      ...
      const stuFees: Partial<Record<StudentFeeHead, number>> = {}
      for (const s of sf) {
        (stuFees as Record<string, number>)[s.fee_head] = Number(s.amount)
      }

      const paidByHead: Partial<Record<FeeHead, number>> = {}
      const paymentRecords: PaymentRecord[] = rawPayments
        .slice()
        .sort((a, b) => b.created_at.localeCompare(a.created_at))
        .map(p => {
          ;(paidByHead as Record<string, number>)[p.fee_head] =
            ((paidByHead as Record<string, number>)[p.fee_head] ?? 0) + Number(p.amount)
          return {
            id: p.id,
      ...
      const feeCalc = calcStudentFee({
        ...
        transportFee,
        payments: paymentRecords.map(p => ({ amount: p.amount })),
      })
